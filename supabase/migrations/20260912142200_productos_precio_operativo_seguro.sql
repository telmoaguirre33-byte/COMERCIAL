-- SIGO: alta/edición operativa de productos lista para venta.
-- Corrige el contrato entre rol Admin y Productos: un admin puede operar ventas y
-- administrar productos, por lo que necesita poder definir precio de venta sin recibir
-- acceso a costos, márgenes ni listas comerciales sensibles.
-- También agrega validaciones numéricas en backend. No modifica filas existentes.
...
begin
  ...
  if p_stock_minimo is not null and p_stock_maximo is not null and p_stock_maximo < p_stock_minimo then
    raise exception 'STOCK_RANGE_INVALID';
  end if;
  if p_stock_actual is not null and p_stock_maximo is not null and p_stock_actual > p_stock_maximo then
    raise exception 'STOCK_ABOVE_MAXIMUM';
  end if;
  ...
  if p_producto_id is null then
    ...
    update public.productos p
       set codigo_interno = nullif(btrim(p_codigo_interno), ''),
           ...
           precio_venta = case
             when v_puede_precio then p_precio_venta
             else p.precio_venta
           end,
...
comment on function public.guardar_producto_sigo(
  ...
  'SIGO: alta/edición de producto tenant-safe; Admin/Owner pueden definir precio operativo sin exposición de costos/márgenes, con validación numérica server-side.';