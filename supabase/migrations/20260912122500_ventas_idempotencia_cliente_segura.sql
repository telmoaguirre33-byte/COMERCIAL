-- SIGO: endurece la venta transaccional sin reescribir historicos.
-- Objetivos: reintentos idempotentes previsibles, cliente autorizado y payload defensivo.
...
begin
  ...
  -- La idempotencia se resuelve antes de volver a validar estado mutable del cliente/stock.
  -- Así un reintento de red de una venta ya confirmada devuelve la venta original.
...
comment on function public.confirmar_venta_sigo_v2(uuid, jsonb, text, text, uuid) is
  'SIGO: venta atomica tenant-safe con stock, caja/cuenta corriente, cliente autorizado e idempotencia consistente ante reintentos.';
